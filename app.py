...
st.set_page_config(page_title="Lets Get Hired", page_icon=":rocket:", layout="wide")
...
DEFAULT_CV = {
    ...
    "summary": "Trust & Safety AI Analyst with 3+ years in LLM evaluation, content safety, abuse detection, and product policy. MSc Business Analytics (Dublin Business School). CSPO certified. Experienced sole market owner across 4 markets.",
...
JOBS = [
    ...
    {"title":"Threat Disruptions Analyst - Trust & Safety EMEA","company":"TikTok","role":"Trust & Safety","source":"TikTok Careers","salary":"38-75k","posted":"30+ days ago","age":31,"url":"https://careers.tiktok.com/position?location=CT_211&query=threat","desc":"Detect and disrupt coordinated inauthentic behaviour."},
    ...
    {"title":"Senior Business Analyst - SMB EMEA","company":"TikTok","role":"Business Analyst","source":"TikTok Careers","salary":"55-75k","posted":"20 days ago","age":20,"url":"https://careers.tiktok.com/position?location=CT_211&query=business+analyst","desc":"Build sales analytics function for EMEA."},
    ...
    {"title":"Business Analytics Manager - YouTube Trust & Safety","company":"YouTube/Google","role":"Data Analyst","source":"Google Careers","salary":"70-95k","posted":"Recent","age":21,"url":"https://careers.google.com/jobs/results/?location=Dublin%2C+Ireland&q=trust+safety","desc":"Define success metrics for YouTube T&S. Data and analytical techniques. Dublin."},
    ...
    {"title":"Privacy and Policy Analyst","company":"Meta","role":"Trust & Safety","source":"Meta Careers","salary":"58-78k","posted":"15 days ago","age":15,"url":"https://www.metacareers.com/jobs?offices=Dublin&q=policy+analyst","desc":"Content policy, regulatory compliance, GDPR, DSA. Dublin office."},
...
COMPANIES = [
    ("Google","G","https://careers.google.com/jobs/results/?location=Dublin&q=trust+safety+analyst"),
    ("Meta","M","https://www.metacareers.com/jobs?offices=Dublin&q=trust+integrity+analyst"),
    ("TikTok","T","https://careers.tiktok.com/position?location=CT_211&query=trust+safety"),
    ("OpenAI","OA","https://openai.com/careers"),
    ("Anthropic","AN","https://www.anthropic.com/careers"),
    ("Stripe","S","https://stripe.com/jobs/search?l=Dublin&q=analyst"),
    ("Cloudflare","CF","https://www.cloudflare.com/careers/jobs/?location=Dublin"),
    ("Microsoft","MS","https://jobs.microsoft.com/en/search?q=analyst&lc=Dublin"),
    ("Amazon","AZ","https://www.amazon.jobs/en/search?location[]=IRL-Dublin"),
    ("HubSpot","HS","https://www.hubspot.com/careers/jobs?q=analyst&countryCodes=IE"),
    ("Revolut","R","https://www.revolut.com/careers/?location=Dublin"),
    ("Accenture","AC","https://www.accenture.com/ie-en/careers/jobsearch?jk=analyst&cl=Dublin"),
    ("Deloitte","DL","https://apply.deloitte.com/careers/SearchJobs/analyst?3_56_3=5440"),
    ("EY","EY","https://www.ey.com/en_ie/careers"),
    ("Salesforce","SF","https://careers.salesforce.com/en/jobs/?search=analyst&location=Dublin"),
    ("LinkedIn","LI","https://careers.linkedin.com/jobs/search?keywords=analyst&location=Dublin"),
    ("AIB","AI","https://aib.ie/careers"),
    ("Bank of Irl","BI","https://careers.bankofireland.com"),
    ("Irish Life","IL","https://www.irishlife.ie/careers"),
    ("Davy","DV","https://www.davy.ie/careers"),
    ("Citi","CI","https://jobs.citi.com/search-jobs/Dublin"),
    ("Intercom","IC","https://www.intercom.com/careers"),
    ("Zendesk","ZD","https://jobs.zendesk.com/us/en/search-results?keywords=analyst&location=Dublin"),
    ("Workday","WD","https://www.workday.com/en-us/company/careers/open-positions.html"),
]

BOARDS = [
    ("LinkedIn","https://www.linkedin.com/jobs/search/?keywords=trust+safety+AI+analyst+product+owner&location=Dublin%2C+Ireland&f_TPR=r86400"),
    ...
    ("Silicon Republic","https://www.siliconrepublic.com/jobs"),
]
...
def role_chip(role):
    ...
    c = m.get(role, "c-sv")
    return f'<span class="chip {c}">{role}</span>'
...
with st.sidebar:
    ...
    page = st.radio("", [
        ...
        "Companies", "Job Boards", "My Tracker",
        "CV Editor", "Interview Prep"
    ])
    st.markdown("---")
    apps = DATA["applications"]
    st.markdown(f"**{len(apps)}** tracked")
    st.markdown(f"**{sum(1 for a in apps if a.get('status')=='Interviewing')}** interviews")
    st.markdown("---")
...
if page == "Dashboard":
    ...
    with col2:
        ...
        if apps:
            ...
            for a in sorted(apps, key=lambda x: x.get("date",""), reverse=True)[:5]:
...
# LIVE JOBS
    ...
    st.markdown("Real roles from company career pages. Green = posted this week.")
    col1, col2 = st.columns(2)
    ...
    else: jobs = sorted(jobs, key=lambda x: -int(x["salary"].split("-")[0].replace("k","000")))
    st.caption(f"{len(jobs)} roles found - sourced from company career pages")
    for j in jobs:
        with st.expander(f"{j['title']} -- {j['company']} -- EUR{j['salary']}"):
            ...
            if c2.button("+ Track", key=f"t_{j['title'][:20]}"):
...
# SILICON REPUBLIC
    ...
    with tab1:
        sr_jobs = [
            ...
            ("Technology Business Analyst - Deloitte Dublin", "https://apply.deloitte.com/careers/SearchJobs/analyst?3_56_3=5440", "Deloitte", "7 days ago"),
        ]
        for title, url, co, dt in sr_jobs:
            st.markdown(f"""<div class="card">
    ...
    with tab2:
        sr_news = [
            ...
            ("Google AI Safety team in Dublin expands with new analyst roles", "https://www.siliconrepublic.com/machines", "2 weeks ago"),
        ]
        for title, url, dt in sr_news:
            st.markdown(f"""<div class="card">
                <a href="{url}" style="font-size:13px;font-weight:500;color:#a78bfa;text-decoration:none">{title}</a>
...
# COMPANIES
    ...
    cos = [(n,a,u) for n,a,u in COMPANIES if not search or search.lower() in n.lower()]
    cols = st.columns(4)
    for i,(name,abbr,url) in enumerate(cos):
        with cols[i%4]:
            st.markdown(f"""<div class="card" style="text-align:center;padding:0.8rem">
                <div style="width:32px;height:32px;border-radius:50%;background:#2d1063;color:#a78bfa;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:600;margin:0 auto 4px">{abbr}</div>
                <div style="font-size:12px;font-weight:500;margin-bottom:5px">{name}</div>
            </div>""", unsafe_allow_html=True)
...
# TRACKER
    ...
    with st.expander("+ Log new application", expanded=False):
        ...
        np_str = st.text_input("Posted date", placeholder="e.g. 3 days ago, 13 Apr 2026")
...
# INTERVIEW PREP
    ...
    st.markdown("STAR answers tailored to your background - built for your job search.")
    ...
    qa_sets = [
        [("Tell me about yourself","3+ years Trust & Safety at Meta via Covalen. Sole market owner across 4 markets. LLM evaluation, abuse detection, content policy. MSc Business Analytics. CSPO. Now seeking a permanent senior T&S role at an AI-first company like OpenAI."),
         ...
         ("Describe a complex T&S case you owned","Use the coordinated account network investigation: SQL analysis to identify signal, cross-team escalation, policy recommendation, enforcement action, and measurable outcome. Quantify impact."),
        ...
        [("Why TikTok?","TikTok has one of the most complex T&S environments globally. Short-form video at scale, multilingual markets, live content, e-commerce. My EMEA market ownership and LLM evaluation experience is directly applicable."),
        ...
        [("Tell me about yourself - ADI framing","Frame as a data analyst specialising in trust signals: I have spent 3 years using SQL and Python to identify coordinated account networks, abuse patterns, and policy violations at Meta. My work sits at the intersection of data analysis and integrity operations."),
         ("Walk me through a SQL analysis","Coordinated account detection: GROUP BY device fingerprint, COUNT of accounts, suspicious temporal clustering. Found networks of fake accounts using shared infrastructure. Escalated to policy team with full evidence package."),
        ...
        [("You worked for Meta before - what would you do differently?","Focus more on proactive rather than reactive enforcement. Build better tooling for market owners. Invest in cross-market knowledge sharing and pattern recognition."),
         ("How do you prioritise across multiple markets?","Risk-based scoring: severity x volume x regulatory exposure. Highest severity gets immediate attention. I owned 4 markets and used a triage matrix to allocate effort efficiently.")],
        [("Why are you leaving / what happened?","My contract with Covalen concluded as Meta consolidated its vendor operations. I am now actively pursuing permanent senior roles in T&S and AI analysis where I can have long-term impact."),
         ("Biggest strength?","Combining technical data skills with operational T&S judgment. I can write the SQL query AND translate findings into a policy recommendation AND present it to senior stakeholders."),
         ("Where in 3 years?","Senior T&S Analyst or AI Policy Manager at a major platform, owning a product area end-to-end and contributing to AI governance frameworks like the EU AI Act.")],
    ]
        ...
        with tab:
            for q, a in qa_list:
                with st.expander(f"? {q}"):
                    st.markdown(a)
                    st.text_area("Your notes", key=f"note_{q[:15]}", height=70, placeholder="Add your own notes...")
    st.markdown("---")
